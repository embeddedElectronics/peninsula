...
FAT32_ReadSector:
        PUSH     {R7,LR}
//   56  return((uint8)disk_read(0,buf,addr,1));  //替换成实际存储器的扇区读函数，这里是SD卡扇区读函数
        MOVS     R3,#+1
        MOVS     R2,R0
        MOVS     R0,#+0
        BL       disk_read
...
FAT32_WriteSector:
        PUSH     {R7,LR}
//   72  return((uint8)disk_write(0,buf,addr,1)); //替换成实际存储器的扇区写函数，这里是SD卡扇区写函数
        MOVS     R3,#+1
        MOVS     R2,R0
        MOVS     R0,#+0
        BL       disk_write
...
??FAT32_Update_FSInfo_Free_Clu_0:
        ...
        STR      R0,[SP, #+0]
//  279 
//  280  ((struct FSInfo *)FAT32_Buffer)->Free_Cluster[0]=((unsigned char *)&Free_Clu)[0]; 
??FAT32_Update_FSInfo_Free_Clu_1:
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+0]
        STRB     R1,[R0, #+488]
//  281  ((struct FSInfo *)FAT32_Buffer)->Free_Cluster[1]=((unsigned char *)&Free_Clu)[1];
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+1]
        STRB     R1,[R0, #+489]
//  282  ((struct FSInfo *)FAT32_Buffer)->Free_Cluster[2]=((unsigned char *)&Free_Clu)[2];
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+2]
        STRB     R1,[R0, #+490]
//  283  ((struct FSInfo *)FAT32_Buffer)->Free_Cluster[3]=((unsigned char *)&Free_Clu)[3];
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+3]
        STRB     R1,[R0, #+491]
...
//  299  FAT32_ReadSector(1+pArg->BPB_Sector_No,FAT32_Buffer);  
        ...
        ADDS     R0,R0,#+1
        BL       FAT32_ReadSector
//  300  ((struct FSInfo *)FAT32_Buffer)->Last_Cluster[0]=((unsigned char *)&Last_Clu)[0]; 
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+0]
        STRB     R1,[R0, #+492]
//  301  ((struct FSInfo *)FAT32_Buffer)->Last_Cluster[1]=((unsigned char *)&Last_Clu)[1];
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+1]
        STRB     R1,[R0, #+493]
//  302  ((struct FSInfo *)FAT32_Buffer)->Last_Cluster[2]=((unsigned char *)&Last_Clu)[2];
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+2]
        STRB     R1,[R0, #+494]
//  303  ((struct FSInfo *)FAT32_Buffer)->Last_Cluster[3]=((unsigned char *)&Last_Clu)[3];
        LDR.W    R0,??DataTable13
        LDRB     R1,[SP, #+3]
        STRB     R1,[R0, #+495]
...
// 1184 
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
// 1185 int8 FAT32_Create_Rec(struct FileInfoStruct *pfi,uint32 cluster,int8 *name,uint8 is_dir,uint8 *ptd)
// 1186 {
FAT32_Create_Rec:
...
// 1476   if(FAT32_Create_Rec(pfi,temp_dir_cluster,temp_dir_name,1,ptd)==-1)
        ...
        BL       FAT32_Create_Rec
        CMN      R0,#+1
        BNE.N    ??FAT32_Create_Dir_0
// 1477   {
// 1478    return 1;
        MOVS     R0,#+1
        B.N      ??FAT32_Create_Dir_2
// 1479   }
// 1480  }
// 1481  return 0;
??FAT32_Create_Dir_1:
...
// 1503    if(FAT32_Create_Rec(pfi,temp_dir_cluster,temp_dir_name,0,ptd)==-1)
        ...
        BL       FAT32_Create_Rec
        CMN      R0,#+1
        BNE.N    ??FAT32_Create_File_2
// 1504    {
// 1505     return 3;
        MOVS     R0,#+3
        B.N      ??FAT32_Create_File_3
// 1506    }    
// 1507   }
// 1508   else
// 1509   {
...
// 7 258 bytes in section .text
// 
// 7 250 bytes of CODE memory (+ 8 bytes shared)
