...
uint8 GetCardStat(void) { return Stat; }
void SetCardStat(uint8 state) { Stat = state; }
...
DRESULT disk_initialize (unsigned char drv)
{
    ...
    //检测并初始化SD卡
    if (ESDHC_OK != hw_sdhc_ioctl (IO_IOCTL_ESDHC_INIT, NULL))
        return RES_ERROR;

    //获取SD卡类型
    param = 0;
    if (ESDHC_OK != hw_sdhc_ioctl (IO_IOCTL_ESDHC_GET_CARD, &param))
        return RES_ERROR;
    ...
    command.READ     = FALSE;
    command.BLOCKS   = 0;
    if (ESDHC_OK != hw_sdhc_ioctl (IO_IOCTL_ESDHC_SEND_COMMAND, &command))
        return RES_ERROR;
    ...
    if (ESDHC_BUS_WIDTH_4BIT == sdcard_ptr->BITS)
    {
        ...
        if (ESDHC_OK != hw_sdhc_ioctl (IO_IOCTL_ESDHC_SET_BUS_WIDTH, &param))
            return RES_ERROR;
    }
...
DRESULT disk_read(uint8 drv,uint8 *buff,uint32 sector, uint8 count)
{
    ...
    if (count == 1)//读单个扇区
    {
        ...
        if (ESDHC_OK == hw_sdhc_ioctl (IO_IOCTL_ESDHC_SEND_COMMAND, &command))
        {
            ...
            if (hw_sdhc_receive_block(buff, IO_SDCARD_BLOCK_SIZE))
                count = 0;
        }
    }
    else //读多个扇区
    {     
        ...
        if (ESDHC_OK == hw_sdhc_ioctl (IO_IOCTL_ESDHC_SEND_COMMAND, &command))
        {
            if (hw_sdhc_receive_block(buff, IO_SDCARD_BLOCK_SIZE*count))
                count = 0;
        }
    }
...
DRESULT disk_write(uint8 drv,const uint8 *buff,uint32 sector,uint8  count)
{
    ...
    if (count == 1)	//写单个扇区
    {
        ...
        if (ESDHC_OK == hw_sdhc_ioctl (IO_IOCTL_ESDHC_SEND_COMMAND, &command))
        {
            ...
            if (hw_sdhc_send_block(buff, IO_SDCARD_BLOCK_SIZE))
                count = 0;
        }
    }
    else //写多个扇区
    {
        ...
        if (ESDHC_OK == hw_sdhc_ioctl (IO_IOCTL_ESDHC_SEND_COMMAND, &command))
        {
            if (hw_sdhc_send_block(buff,IO_SDCARD_BLOCK_SIZE*count))
                count = 0;
        }
    }
